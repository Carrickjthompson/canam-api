# main.py — Can-Am Specialist API (API endpoints + GPT-parity /chat)
...
# ---------- GPT PARITY: proxy /chat directly to your Assistant ----------
...
def chat(req: ChatIn):
    ...
    if not asst_id:
        raise HTTPException(status_code=500, detail="ASSISTANT_ID not set.")

    try:
        # Responses API: returns the exact output your Assistant (your GPT) would produce
        ...
        if not text:
            # Fallback for SDKs that return a structured output list
            chunks = []
            ...
            text = "\n".join(chunks).strip() or "No answer."
        return ChatOut(answer=text.strip())
